...
function simpleCommand($command, $message)
{
  ...
  switch ($commandCall) {
    case $prefix . 'eita':
      $eitaCounter++;
      $message->reply('A Levxyca já falou eita ' . $eitaCounter . ' vezes');
      break;
    case $prefix . 'caraio':
      $caraioCounter++;
    ...
    case $prefix . 'cancelar':
      if (sizeof($command) != 2) {
        $message->reply('Vou te cancelar por ter mandado o comando errado!');
      } else {
        $cancelamento = getCancelamento();
        $message->reply($command[1] . ' foi cancelado(a) por ' . $cancelamento);
      }
      break;
    case $prefix . 'amor':
      $message->reply(
